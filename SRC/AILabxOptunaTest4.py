...
参数表达式a1 * ts1 +a2 * ts2 + b1*rs1 + b2 * rs2 + c1* ms1/ms2 + c2 * ms21/ms22
...
def objective(trial):
    ...
    win_trend_score1 = trial.suggest_int('win_trend_score1', 15, 50) # default: 25
    win_trend_score2 = trial.suggest_int('win_trend_score2', 15, 50) # default: 25
    ...
    win_ma_score21 = trial.suggest_int('win_ma_score21', 2, 15) # default: 5
    win_ma_score22 = trial.suggest_int('win_ma_score22', 8, 50) # default: 18
    ...
    def __call__(self, study, trial):
        if trial.state == optuna.trial.TrialState.COMPLETE:
            ...
            if len(self.results) % self.save_interval == 0:
                write_to_file(self.results, f'./data/AILabxOptunaTest4_interim_{len(self.results)}.xlsx')
...
if __name__ == '__main__':
    # 使用数据库存储确保进程安全
    storage_url = "sqlite:///AILabxOptunaStudy4_001.db"
    ...
    # 保存到文件
    write_to_file(final_results, f'./data/AILabxOptunaTest4_final_{len(final_results)}.xlsx')
    ...
    best_params_df.to_excel(f'./data/AILabxOptunaTest4_best_params_{timestamp0}.xlsx', index=False)
